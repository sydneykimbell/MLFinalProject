...
def euclidean_distance(df):
    sentence_1 = df['sentence_1'].values
    sentence_2 = df['sentence_2'].values
    feature = pd.DataFrame(columns=['euclidean_distance'])
    euclid_dist = []
    for s1, s2 in zip(sentence_1, sentence_2):
        l1 = []
        l2 = []
        ...
        euclid_dist.append(math.sqrt(sum((e1-e2)**2 for e1, e2 in zip(l1,l2))))
    
    feature['euclidean_distance'] = euclid_dist
    return feature
...
for i in range(len(X_test_id)):
    file.write(X_test_id[i] + '\t' + str(test_pred[i]) + '\n')